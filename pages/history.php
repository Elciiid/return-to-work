...
<body class="font-sans text-gray-800 flex flex-col md:flex-row min-h-screen">
        ...
        <div class="flex-1 overflow-y-auto pr-4 space-y-6 history-scroll-area pb-[8rem] custom-scrollbar">
            <?php if (empty($submissions)): ?>
                <div class="text-center py-20 bg-white/50 rounded-[3rem] border border-dashed border-pink-200">
                    ...
                    <p class="font-bold text-gray-400">No records found for this period.</p>
                </div>
            <?php else: ?>
                <?php foreach ($submissions as $row): ?>
                    <div class="glass-card rounded-[2rem] p-8 shadow-sm group transition-all hover:shadow-md">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-6 pt-6 border-t border-pink-50">
                            <div class="text-left">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-sm font-bold text-gray-700">
                                    ...
                                    if ($nurseDecl === null) {
                                        ...
                                        $declColor = 'text-gray-500';
                                        if ($nurseDecl === 'fit to work')
                                            $declColor = 'text-black-600';
                                        else if ($nurseDecl === 'unfit to work')
                                            $declColor = 'text-red-600';
                                        else if ($nurseDecl === 'return to work')
                                            $declColor = 'text-black-600';
            ...
            <div class="p-10 grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                <div class="space-y-6 md:col-span-2">
                    ...
                    <div class="p-6 rounded-3xl border border-rose-200 bg-rose-50/50">
                        <label class="block text-[10px] font-black text-rose-400 uppercase mb-2">Approval
                            Details</label>
                        <p class="text-rose-700 font-medium leading-relaxed">Application has been approved and is ready
                            to return to work.</p>
                        <div class="mt-3 pt-3 border-t border-rose-200">
                            <p class="text-xs text-rose-600">
                                <strong>Approved by:</strong> <span id="modalApprovedBy"></span><br>
                                <strong>Approved on:</strong> <span id="modalApprovedDate"></span>
                            </p>
                        </div>
                    </div>
                </div>
                <div class="space-y-4">
                    ...
                    <div class="p-4 rounded-2xl bg-rose-50 border border-rose-100"><label
                        ...
                        <p class="font-bold text-rose-600">Approved</p>
                    </div>
    ...
    <script>
        ...
        function openDetails(data) {
            ...
            document.getElementById('modalTitle').innerText = 'Application #' + data.id.toString().padStart(4, '0') + ' - Approved';
            ...
            if (nurseDecl === null) {
                ...
                declColor = 'text-gray-500';
                if (nurseDecl === 'fit to work') declColor = 'text-black';
                else if (nurseDecl === 'unfit to work') declColor = 'text-red-600';
                else if (nurseDecl === 'return to work') declColor = 'text-blue-600';
            }
            ...
            document.getElementById('modalApprovedBy').innerText = data.approved_by || '—';
            document.getElementById('modalApprovedDate').innerText = data.approved_at ? new Date(data.approved_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
