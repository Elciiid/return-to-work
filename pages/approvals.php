    ...
    <main
        ...
        <form method="GET" class="glass-panel rounded-[2rem] p-6 shadow-sm border border-white mb-8 shrink-0">
            <div class="grid grid-cols-1 md:grid-cols-6 gap-6 items-end">
                ...
                <div>
                    <label class="text-[10px] font-black text-gray-400 uppercase ml-1 mb-1 block">Status</label>
                    <select name="status"
                        ...
                        <option value="Pending" <?= $status_filter === 'Pending' ? 'selected' : '' ?>>Pending Only
                        </option>
                        <option value="Approved" <?= $status_filter === 'Approved' ? 'selected' : '' ?>>Approved Only
                        </option>
                        <option value="Declined" <?= $status_filter === 'Declined' ? 'selected' : '' ?>>Declined Only
                        </option>
                    </select>
                </div>
                <div>
        ...
        <div class="flex-1 overflow-y-auto pr-4 space-y-6 history-scroll-area pb-[8rem] custom-scrollbar">
            <?php if (empty($submissions)): ?>
                <div class="text-center py-20 bg-white/50 rounded-[3rem] border border-dashed border-pink-200">
                    ...
                    <p class="font-bold text-gray-400">No applications found.</p>
                </div>
            <?php else: ?>
                        ...
                        <div class="flex justify-between items-start mb-6">
                            <div class="flex items-center gap-4">
                                ...
                                if ($row['status'] === 'Approved') {
                                    ...
                                    $statusColor = 'bg-rose-100 text-rose-500';
                                    $statusIcon = 'fa-times';
                                ...
                                ?>
                                <div
                                    class="w-12 h-12 <?= $statusColor ?> rounded-2xl flex items-center justify-center shadow-inner">
                                ...
                                <div>
                                    ...
                                    <!-- Status Badge for Non-Pending or if viewed in All -->
                                    <p class="mt-1">
                                        <?php if ($row['status'] === 'Approved'): ?>
                                            <span
                                                class="text-[10px] font-black uppercase text-pink-500 bg-pink-50 px-2 py-0.5 rounded-full border border-pink-100">Approved</span>
                                        <?php elseif ($row['status'] === 'Declined'): ?>
                                            <span
                                                class="text-[10px] font-black uppercase text-rose-500 bg-rose-50 px-2 py-0.5 rounded-full border border-rose-100">Declined</span>
                                        <?php elseif ($row['status'] === 'Pending'): ?>
                                            <span
                                                class="text-[10px] font-black uppercase text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-full border border-yellow-100">Pending</span>
                                        <?php endif; ?>
                                    </p>
                                </div>
                            </div>
                                ...
                                <?php if ($row['status'] === 'Pending'): ?>
                                    <?php if ($row['nurse_declaration'] === null): ?>
                                        ...
                                        <button disabled
                                            ...
                                            <i class="fa-solid fa-clock mr-1"></i> Waiting for Nurse
                                        </button>
                                    <?php else: ?>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-6 gap-6 pt-6 border-t border-pink-50">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-sm font-bold text-gray-700">
                                    ...
                                    if ($nurseDecl === null) {
                                        echo '<span class="text-orange-600">Not Declared</span>';
                                    } else {
        ...
        <?php if ($total_pages > 1): ?>
            <div class="pagination-capsule flex items-center gap-1">
                ...
                <?php if ($start > 1): ?>
                    <a href="?page=1&search=<?= urlencode($search) ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>&sort=<?= $sort_order ?>&status=<?= $status_filter ?>"
                ...
                <?php if ($page > 1): ?>
                    <a href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>&sort=<?= $sort_order ?>&status=<?= $status_filter ?>"
                ...
                <?php for ($i = $start; $i <= $end; $i++): ?>
                    <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>&sort=<?= $sort_order ?>&status=<?= $status_filter ?>"
                ...
                <?php if ($end < $total_pages): ?>
                    <a href="?page=<?= $total_pages ?>&search=<?= urlencode($search) ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>&sort=<?= $sort_order ?>&status=<?= $status_filter ?>"
        ...
        <div class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden">
            ...
            <div class="p-10 grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                <div class="space-y-6 md:col-span-2">
                    ...
                    <!-- Additional Details for Approved/Declined -->
                    <div id="modalStatusDetails" class="hidden">
                ...
                <div class="space-y-4">
                    ...
                    <div id="modalStatusCard" class="p-4 rounded-2xl bg-gray-50 border border-gray-100"><label
                            class="text-[9px] font-black text-gray-400 uppercase">Status</label>
                        <p id="modalStatusText" class="font-bold text-gray-600"></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function openDetails(data) {
            ...
            if (data.status === 'Approved') {
                ...
                statusCard.className = 'p-4 rounded-2xl bg-rose-50 border border-rose-100';
            } else {
            ...
            if (data.status === 'Approved') {
                ...
                detailsDiv.innerHTML = `
                    <div class="p-6 rounded-3xl border border-pink-200 bg-pink-50/50">
                        <label class="block text-[10px] font-black text-pink-400 uppercase mb-2">Approval Details</label>
                        <p class="text-pink-700 font-medium leading-relaxed">Application has been approved.</p>
                        <div class="mt-3 pt-3 border-t border-pink-200">
                             <p class="text-xs text-pink-600">
                                <strong>Approved by:</strong> ${data.approved_by || '—'}<br>
                                <strong>Approved on:</strong> ${data.approved_at || '—'}
                            </p>
                        </div>
                    </div>`;
            } else if (data.status === 'Declined') {
                ...
                detailsDiv.innerHTML = `
                    <div class="p-6 rounded-3xl border border-rose-200 bg-rose-50/50">
                        ...
                        <p class="text-rose-700 font-medium leading-relaxed">${data.decline_reason || 'No reason provided'}</p>
                        <div class="mt-3 pt-3 border-t border-rose-200">
                             <p class="text-xs text-rose-600">
                                <strong>Declined by:</strong> ${data.declined_by || '—'}<br>
                                <strong>Declined on:</strong> ${data.declined_at || '—'}
                            </p>
                        </div>
                    </div>`;
            } else {
